...
router.post('/', auth.check, (req, res, next) => {
	...
	.then((bill) => {
		...
		if(bill) {
			...
			else res.status(500).end('only presenter is authorized to begin polling');
		}
	...
	.then(async (bill) => {
		...
		if(bill) {
			...
			for(sub of bill.subscribers) {
				...
				let header = '<h2>Bill ' + bill.no + ': ' + bill.name + ' has begun polling.</h2>'
				let body =
				...
				if(sub.status === 'presenting') {
					content = header.concat('<h4>You are a member of the party ' + bill.party.name + ', the party that presented this bill.</h4>', body);
				}

				//for members of the supporting party
				else if(sub.status === 'supporting') {
					let party = await Party.findById(sub.mla.party, 'name');
					content = header.concat('<h4>You are a member of the party ' + party.name + ', a party supporting this bill.</h4>', body);
				}

				//for presenter of the bill
				else if(sub.status === 'presenter') {
					content = header.concat('<h4>You have presented this bill in the assembly.</h4>', body);
				}

				//for other regular subscribers
				else if(sub.status === 'regular') {
					content = header.concat('<h4>You have subscribed to updates for this bill.</h4>', body);
				}

				let mailCall = (callback) => {
					emailing.nodemailerEmail(subject, content, email)
	...
	.then((results) => {
		if(results) { //to deal with when promise isnt resolved(i.e. when mails aren't sent)
