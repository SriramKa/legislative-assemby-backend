...
router.get('/', auth.check, (req, res, next) => {
	...
	if(!req.query.presented) res.status(200).json(user);
		...
		functions.displayBills(Bill.find({_id: {$in: user.presented}}, 'name no description party represented status'), 0, 1, res);
	}
});
	...
	bcrypt.compare(req.body.password, user.password, async (err, isMatch) => {
		if(err) throw err;
			...
			if(idCheck || usernameCheck) {
				if(idCheck) res.write('this MLA ID is already taken\n');
				if(usernameCheck) res.write('this username is already taken\n');
				res.status(200).end();
			}
			//updating
			else {
...
router.post('/presentbill', auth.check, async (req, res, next) => {
	...
	toInsert.subscribers = [{ //adding presenter to subscriber list
		mla: user._id,
		status: 'presenter',
		voted: false,
		vote: false
	...
	let modificationCalls = [
		...
		(callback) => {
			Party.findOne({name: toInsert.party}, '_id members')
			.then((party) => {
				toInsert.party = party._id; //set as presenting party
				//adding members of presenting party to subscriber list
				party.members.forEach((member) => {
					if(member.toString() !== user._id.toString()) toInsert.subscribers.push({
						mla: member,
						status: 'presenting',
						voted: false,
						vote: false
		...
		(callback) => {
			Party.find({name: {$in: toInsert.supporting}}, '_id members')
			.then((parties) => {
				...
				parties.forEach((supporter) => {
					...
					if(supporter._id.toString() !== user.party._id.toString()) {
						...
						supporter.members.forEach((member) => toInsert.subscribers.push({
							mla: member,
							status: 'supporting',
							voted: false,
							vote: false
	...
	async.parallel(modificationCalls)
	.then((result) => {
	...
	.then((bill) => {
		...
		let updateCalls = [
			...
			(callback) => {
				Party.findByIdAndUpdate(bill.party, {$push: {presented: bill._id}})
				.then((party) => callback(null, party));
			},
			//updating 'supporting' of party
			(callback) => {
				Party.updateMany({_id: {$in: bill.supporting}}, {$push: {supporting: bill._id}}).then((party) => callback(null, party));
			},
			//updating 'presented' of mla
			(callback) => {
				Mla.findByIdAndUpdate(user._id, {$push: {presented: bill._id}})
...
router.post('/poll', auth.check, (req, res, next) => {
	...
	.then((bill) => {
		...
		//implementing the vote
			...
			let voterIndex = bill.subscribers.findIndex((sub) => (sub.mla.toString() === user._id.toString()));
			if(voterIndex !== -1) {
	...
	.then((bill) => {
		if(bill) { //to handle in case bill.save isn't called(ie the promise is never made)
			res.status(200).json({
